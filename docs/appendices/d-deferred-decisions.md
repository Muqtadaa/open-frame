...
| **Auth provider**                                   | Providers change fast, and the capability boundary — the part that matters — is already designed. Now a two-way choice: Clerk, or Supabase Auth on the project the database already needs. Narrowed by the access model below: an account is for ownership, not for entry, so this buys less than it used to | Phase 4 Stage 3 |
| ~~**How someone gets into a board**~~               | ✅ **Closed: link, then an optional per-board password, with named guests — not accounts.** A guest identity is a label in awareness and never an authorization; a password gates a room without encrypting it, and is exchanged for a short-lived token rather than travelling in a URL. Needs an ADR when the work starts | [Phase 4](../phases/phase-4-collaboration.md) Stage 2 |
