...
/**
 * The ground a text colour will be read against, as a hex, or `null` when
 * there is no single answer.
 *
 * Resolved through the live document so it follows the theme: a token is a
 * variable until the cascade has been asked. `null` — a mixed selection — is
 * why the picker's contrast line is optional rather than always shown; a
 * number measured against a surface only some of the selection has is worse
 * than no number.
 */
function groundOf(surface: ColorValue | undefined): HexColor | null {
 ...
 *
 * One component for both colour rows: they differ in what a token MEANS (a
 * wash to stand on, an ink to write with) and in nothing else, so a second
 * copy would be a second place to fix anything either of them gets wrong.
 *
